...
const imagePlaceholders = [
  { id: 1, label: "Image 1", color: "#EBF3ED" },
  { id: 2, label: "Image 2", color: "#DCE8DF" },
  { id: 3, label: "Image 3", color: "#D0DED3" },
  { id: 4, label: "Image 4", color: "#C4D4C8" },
];
...
function HeartSparkleIcon() {
  return (
    ...
    >
      <g clipPath="url(#clip0_intro_img)">
        <path
...
/* ─── Static Convo Screen (Frame 2) ──────────────────────── */

function StaticExpandingMenu() {
  return (
    <div
...
function AnimatedConvoScreen() {
  ...
  const chipCount = responseChips.length;
  const textDelay = chipBaseDelay + chipCount * chipStagger + 0.3;

  return (
    <>
      {/* Phase 1: Menu + Person icon — fade in with the frame */}
      <motion.div
        className="absolute inset-0"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.4, delay: navDelay }}
      >
        <StaticTopNavigation />
      </motion.div>

      {/* Phase 1: Ask Ivy prompt bar — fade in with the frame */}
      <motion.div
        className="absolute inset-0"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      {/* Phase 2: Chips — stagger bottom-first with subtle bounce */}
      <div className="absolute left-1/2 -translate-x-1/2 top-[469px] w-[335px] flex flex-col gap-[4.08px]">
        {responseChips.map((text, index) => {
          const reverseIndex = chipCount - 1 - index;
          const delay = chipBaseDelay + reverseIndex * chipStagger;
      ...
      {/* Phase 3: Headline + Care Team badge */}
      <motion.div
        className="absolute inset-0"
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{
          opacity: { duration: 0.45, delay: textDelay },
          y: { duration: 0.5, delay: textDelay, ease: [0, 0, 0.58, 1] },
        }}
      >
        <StaticHeadline />
        <StaticCareTeamBadge />
...
export function IntroImageRandomDemo({
  ...
  const [frame, setFrame] = useState<"image" | "convo">("image");
  ...
  useEffect(() => {
    setFrame("image");
    const timer = setTimeout(() => setFrame("convo"), 2500);
          ...
          {frame === "image" ? (
            ...
            >
              <span
                ...
                style={{ color: "rgba(22, 43, 51, 0.35)" }}
              >
                {selectedImage.label}
              </span>
            </motion.div>

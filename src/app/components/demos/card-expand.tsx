...
function PrescriptionCard({
  ...
  // Anticipation squeeze before expand
  ...
  return (
    <motion.div
      ...
      onClick={handleClick}
      initial={{ left: 21, top: 275, right: 103, bottom: 234, rotateX: 0, scale: 1 }}
      animate={{
        left: expanded ? [21, 20, 0] : [0, 20, 21],
        top: expanded ? [275, 20, 0] : [0, 20, 275],
        right: expanded ? [103, 20, 0] : [0, 20, 103],
        bottom: expanded ? [234, 20, 0] : [0, 20, 234],
        rotateX: expanded ? [0, -25, 0] : [0, -18, 0],
      ...
      transition={{
        default: {
          duration: 1.5,
          times: [0, 0.35, 1],
          ease: [[0.32, 0.5, 0.25, 1], [0.18, 0.0, 0.25, 1]],
        },
        rotateX: {
          duration: 1.5,
          ease: [0.22, 0.68, 0.36, 1],
          times: expanded ? [0, 0.2, 0.5] : [0, 0.2, 0.5],
        },
        scale: {
          type: "spring",
          stiffness: 500,
    ...
    >
      ...
      <motion.div
        ...
        initial={{ clipPath: "inset(0px round 24px)" }}
        animate={{
          clipPath: expanded
            ? ["inset(0px round 24px)", "inset(0px round 20px)", "inset(0px round 0px)"]
            : ["inset(0px round 0px)", "inset(0px round 20px)", "inset(0px round 24px)"],
        }}
        transition={{
          clipPath: {
            duration: 1.5,
      ...
      <motion.div
        ...
        initial={{ width: 376, height: 564, x: "-51%", y: "-44%" }}
        animate={{
          width: expanded ? 676 : [676, 600, 376],
          height: expanded ? 1014 : [1014, 900, 564],
          x: "-51%",
          y: expanded ? "-48%" : ["-48%", "-55%", "-44%"],
        }}
        transition={
          expanded
            ? { duration: 1.5, ease: [0.16, 0.85, 0.1, 1] }
            : {
                duration: 1.5,
                times: [0, 0.35, 1],
                ease: [[0.32, 0.5, 0.25, 1], [0.18, 0.0, 0.25, 1]],
      ...
      <motion.div
        ...
        initial={{ left: 16, right: 11, bottom: 28 }}
        animate={{
          left: expanded ? [16, 20, 22] : [22, 20, 16],
          right: expanded ? [11, 20, 22] : [22, 20, 11],
          bottom: expanded ? [28, 46, 64] : [64, 46, 28],
        }}
        transition={{
          duration: 1.5,
          times: [0, 0.35, 1],
      ...
      >
        <motion.div
          ...
          initial={false}
          animate={{ width: "50%" }}
          transition={{ duration: 1.5, ease: [0.0, 0.0, 0.25, 1] }}
        />
...
export function CardExpandDemo({ replayCount = 0 }: { replayCount?: number }) {
  ...
  /* Show close button ~500ms after expand, then text at ~1000ms */
  useEffect(() => {
    if (expanded) {
      const tClose = setTimeout(() => setShowClose(true), 500);
      const tText = setTimeout(() => setShowText(true), 1000);
  ...
  const handleCollapse = useCallback(() => {
    ...
    collapseTimerRef.current = setTimeout(() => {
      setExpanded(false);
    }, 500); // wait for text + gradient exit
  }, []);
  ...
  const easeTransition = { duration: 1.5, ease: [0.32, 0.72, 0.12, 1] as readonly [number, number, number, number] };
        ...
        <AnimatePresence>
          {showClose && (
            <motion.button
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1, transition: { duration: 0.2, delay: 1.3 } }}

      ...
      <div className="relative w-[119px] h-[48px] rounded-[56px] border border-[rgba(154,189,177,0.2)] bg-[rgba(235,243,237,0.1)]">
        ...
        <div className="absolute left-[2px] top-[3px] w-[57.5px] h-[42px] rounded-[24px] flex items-center justify-center">
          <svg width="19.2" height="19.2" viewBox="0 0 19.2 19.2" fill="none">
            ...
            <defs>
              <clipPath id="clip0_nav_v2"><rect fill="white" height="19.2" width="19.2"/></clipPath>
              <clipPath id="clip1_nav_v2"><rect fill="white" height="9.44922" transform="translate(10.4023 9.19922)" width="9.44922"/></clipPath>
            </defs>
          </svg>
        </div>
...
function PrescriptionCard({
  ...
  // This is completely independent from Motion's declarative system.
  ...
  const playScale = useCallback((keyframes: Keyframe[], duration: number, easing = "ease-out") => {
    ...
    // Cancel any in-progress scale animation
  ...
  // Expand & collapse landing bounces
  ...
  useEffect(() => {
    ...
    if (expanded && !wasExpanded) {
      // Expand landing bounce: subtle overshoot → settle
      const t = setTimeout(() => {
        playScale(
          [
    ...
    if (!expanded && wasExpanded) {
      // Collapse landing bounce: subtle squish → settle
      const t = setTimeout(() => {
        playScale(
          [
            { transform: "scale(1)", offset: 0 },
            { transform: "scale(0.96)", offset: 0.35 },
            { transform: "scale(1.015)", offset: 0.7 },
            { transform: "scale(1)", offset: 1 },
          ],
          350,
          "ease-out"
  ...
  // Collapse anticipation: very subtle breathe out, then snap back
  const prevAnticipateRef = useRef(false);
  useEffect(() => {
    const wasAnticipating = prevAnticipateRef.current;
    prevAnticipateRef.current = anticipateCollapse;

    if (anticipateCollapse && !wasAnticipating) {
      playScale(
  ...
  }, [anticipateCollapse, playScale]);

  const smoothEase = [0.4, 0.0, 0.2, 1] as const;
  const duration = 0.3;
  const positionTransition = { duration, ease: smoothEase };
  ...
  return (
    ...
    >
      ...
      <div ref={bounceRef} className="absolute inset-0" style={{ transform: "scale(1)" }}>
        {/* Inner clip wrapper */}
        <motion.div
          className="absolute inset-0 overflow-hidden bg-[#13262e]"
          initial={false}
          animate={{
            borderRadius: expanded ? 45 : 24,
          }}
          transition={positionTransition}
        >
          {/* Bottle image */}
          <motion.div
            ...
            initial={false}
            animate={{
              width: expanded ? 608 : 376,
              height: expanded ? 913 : 564,
              x: "-51%",
              y: expanded ? "-48%" : "-44%",
            }}
            transition={positionTransition}
...
export function CardExpandV2Demo({ replayCount = 0 }: { replayCount?: number }) {
  const [expanded, setExpanded] = useState(false);
  const [showText, setShowText] = useState(false);
  const [showClose, setShowClose] = useState(false);
  const [anticipateCollapse, setAnticipateCollapse] = useState(false);
  ...
  useEffect(() => {
    ...
    // Don't auto-reset showText here – the staged collapse handler does it
  }, [expanded]);

  /* Staged collapse: text + X disappear first, then card collapses */
  const handleCollapse = useCallback(() => {
    setAnticipateCollapse(true);
    setShowText(false);
    setShowClose(false);
    collapseTimerRef.current = setTimeout(() => {
      setAnticipateCollapse(false);
      setExpanded(false);
    }, 250); // quick exit then collapse
  }, []);
  ...
  const easeTransition = { duration: 0.3, ease: [0.4, 0.0, 0.2, 1] as readonly [number, number, number, number] };
  ...
  return (
    <DemoShell>
      <div className="absolute inset-0 bg-white overflow-hidden">
        ...
        <motion.div
          className="absolute top-[63px] right-[20px] w-[52px] h-[48px] rounded-[56px] border border-[rgba(154,189,177,0.2)] bg-[rgba(235,243,237,0.1)] flex items-center justify-center z-[5]"
          initial={false}
          animate={{ opacity: expanded ? 0 : 1 }}
        ...
        <PrescriptionCard
          ...
          showText={showText}
          anticipateCollapse={anticipateCollapse}
        />
